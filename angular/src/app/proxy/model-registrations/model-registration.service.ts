import type { GetModelRegistrationsInput, ModelRegistrationCreateDto, ModelRegistrationDto, ModelRegistrationUpdateDto } from './models';
...
export class ModelRegistrationService {
  ...
  getList = (input: GetModelRegistrationsInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<ModelRegistrationDto>>({
      method: 'GET',
      url: '/api/app/model-registrations',
      params: { filterText: input.filterText, modelMin: input.modelMin, modelMax: input.modelMax, apiPath: input.apiPath, localPath: input.localPath, scheduleMin: input.scheduleMin, scheduleMax: input.scheduleMax, intervalMin: input.intervalMin, intervalMax: input.intervalMax, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
