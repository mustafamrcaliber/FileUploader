...
* Check the `ConnectionStrings` in `appsettings.json` files under the `FileUploader.AuthServer`, `FileUploader.HttpApi.Host` and `FileUploader.DbMigrator` projects and change it if you need.
...
* Run `FileUploader.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p eae3efe6-b287-42bc-9016-f1a19090d1e8
```

> `eae3efe6-b287-42bc-9016-f1a19090d1e8` is the password of the certificate, you can change it to any password you want.
...
* `FileUploader.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `FileUploader.AuthServer`: ASP.NET Core MVC / Razor Pages application that is integrated OAuth 2.0(`OpenIddict`) and account modules. It is used to authenticate users and issue tokens.
* `FileUploader.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
